...
<<<<<<< HEAD
	...
	inc	hl
	ld	(hl), a
;main.c:80: if(joypad() & J_A) {
	call	_joypad
	bit	4, e
	jr	Z, 00121$
;main.c:81: printf("Tileindex: %d\n", tileindexTL);
	ldhl	sp,	#2
	ld	a, (hl+)
	ld	e, a
	ld	d, (hl)
...
;main.c:414: font_init();
	call	_font_init
;main.c:415: min_font = font_load(font_min);
	ld	de, #_font_min
	push	de
	call	_font_load
	pop	hl
;main.c:416: font_set(min_font);
	push	de
	call	_font_set
	pop	hl
...
;main.c:432: SHOW_SPRITES;
	...
	or	a, #0x02
	ldh	(_LCDC_REG + 0), a
;main.c:433: DISPLAY_ON;
	ldh	a, (_LCDC_REG + 0)
	or	a, #0x80
	ldh	(_LCDC_REG + 0), a
;main.c:435: UINT8 turn_count = 0;
	ldhl	sp,	#1
;main.c:436: UINT8 move_count = 0;  
	xor	a, a
	ld	(hl+), a
	ld	(hl), a
;main.c:438: while(1){
00133$:
;main.c:439: if (car1.vel == 0) {
	ld	hl, #_car1 + 7
	ld	e, (hl)
	ld	a, e
	or	a, a
	jr	NZ, 00104$
;main.c:440: turn_count = 2;
	ldhl	sp,	#1
	ld	(hl), #0x02
	jr	00105$
00104$:
;main.c:441: } else if (turn_count == 0) {
	ldhl	sp,	#1
	ld	a, (hl)
	or	a, a
	jr	NZ, 00105$
...
;main.c:467: hit_ball();
	call	_hit_ball
00111$:
;main.c:474: if(joypad() & J_B){
	call	_joypad
;main.c:475: car1.acc = -1;
	ld	bc, #_car1 + 8
;main.c:474: if(joypad() & J_B){
	bit	5, e
	jr	Z, 00116$
;main.c:475: car1.acc = -1;
	...
	jr	00117$
00116$:
;main.c:476: } else if (joypad() & J_A){
	call	_joypad
	bit	4, e
	jr	Z, 00113$
;main.c:477: car1.acc = 1;
	ld	a, #0x01
	ld	(bc), a
	jr	00117$
00113$:
;main.c:479: car1.acc = 0;
	xor	a, a
	ld	(bc), a
00117$:
...
;main.c:482: if (car1.direction == 0) {
	...
	or	a, a
	jr	NZ, 00119$
;main.c:483: car1.direction = 15;
	ld	hl, #_car1
	ld	(hl), #0x0f
	jr	00120$
00119$:
;main.c:485: car1.direction -= 1;
	dec	a
	ld	(#_car1),a
00120$:
...
;main.c:490: if (car1.direction == 15) {
	...
	sub	a, #0x0f
	jr	NZ, 00125$
;main.c:491: car1.direction = 0;
	ld	hl, #_car1
	ld	(hl), #0x00
	jr	00126$
00125$:
;main.c:493: car1.direction += 1;
	ldhl	sp,	#0
	ld	a, (hl)
	inc	a
	ld	(#_car1),a
